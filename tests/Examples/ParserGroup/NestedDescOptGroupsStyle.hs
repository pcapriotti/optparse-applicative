module Examples.ParserGroup.NestedDescOptGroupsStyle (opts, myPrefs, main) where
...
opts =
  info
    ...
    ( progDesc "Nested parser groups"
        <> header "parser_group.nested_desc_opt_groups_style - a test for optparse-applicative"
    )

main :: IO ()
main = do
  r <- customExecParser myPrefs opts
  print r

myPrefs :: ParserPrefs
myPrefs = prefs (helpShowGlobals <> briefDescOpt (BriefDescOptGroups $ Just styleFn))

-- Custom style fn, skip some groups.
styleFn :: String -> Maybe Doc
styleFn "First group" = Nothing
styleFn "Second group" = Nothing
styleFn "Group 2" = Nothing
styleFn g = Just . Doc.angles . Doc.pretty . fmap Ch.toUpper . dropColon $ g
