module Examples.ParserGroup.NestedDescOptGroups (opts, myPrefs, main) where
...
opts =
  info
    ...
    ( progDesc "Nested parser groups"
        <> header "parser_group.nested_desc_opt_groups - a test for optparse-applicative"
    )
...
myPrefs :: ParserPrefs
myPrefs = prefs (helpShowGlobals <> briefDescOpt (BriefDescOptGroups Nothing))
