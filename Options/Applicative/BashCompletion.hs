...
zshCompletionScript prog progn = return
  ...
  , ""
  , "  if [[ -n $parts[2] ]]; then"
  , "    # We have a description coming from the rich optparse"
  , "    # implementation. Write its description in parentheses"
  , "    # including the original word too (it's all that shown)."
