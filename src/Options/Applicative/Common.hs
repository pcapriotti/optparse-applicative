...
optionNames :: OptReader ann a -> [OptName]
...
liftOpt :: Option ann a -> Parser ann a
liftOpt = OptP

optMatches :: MonadP ann m => Proxy ann -> Bool -> OptReader ann a -> OptWord -> Maybe (StateT Args m a)
optMatches proxy disambiguate opt (OptWord arg1 val) = case opt of
  OptReader names rdr no_arg_err -> do
    guard $ has_name arg1 names
    Just $ do
      args <- get
      let mb_args = uncons $ maybeToList val ++ args
      let missing_arg = missingArgP proxy (no_arg_err $ showOption arg1) (crCompleter rdr)
      (arg', args') <- maybe (lift missing_arg) return mb_args
      put args'
      lift $ runReadM proxy (withReadM (errorFor arg1) (crReader rdr)) arg'
...
searchParser :: Monad m
             => (forall r . Option ann r -> NondetT m (Parser ann r))
             -> Parser ann a -> NondetT m (Parser ann a)
...
searchOpt :: MonadP ann m => ParserPrefs -> OptWord -> Parser ann a
          -> NondetT (StateT Args m) (Parser ann a)
searchOpt pprefs w = searchParser $ \opt -> do
  let disambiguate = prefDisambiguate pprefs
                  && optVisibility opt > Internal
  case optMatches Proxy disambiguate (optMain opt) w of
    Just matcher -> lift $ fmap pure matcher
    Nothing -> mzero

searchArg :: forall ann m a. MonadP ann m => ParserPrefs -> String -> Parser ann a
          -> NondetT (StateT Args m) (Parser ann a)
searchArg prefs arg =
  searchParser $ \opt -> do
      ...
      CmdReader _ _ f ->
        case (f arg, prefBacktrack prefs) of
          (Just subp, NoBacktrack) -> lift $ do
            args <- get <* put []
            fmap pure . lift $ enterContext arg subp *> runParserInfo subp args <* exitContext (Proxy @ann)

          (Just subp, Backtrack) -> fmap pure . lift . StateT $ \args ->
            enterContext arg subp *> runParser (infoPolicy subp) CmdStart (infoParser subp) args <* exitContext (Proxy @ann)
          ...
          (Nothing, _)  -> mzero
      ArgReader rdr ->
        fmap pure . lift . lift $ runReadM (Proxy @ann) (crReader rdr) arg
      _ -> mzero

stepParser :: MonadP ann m => ParserPrefs -> ArgPolicy -> String
           -> Parser ann a -> NondetT (StateT Args m) (Parser ann a)
...
runParser :: MonadP ann m => ArgPolicy -> IsCmdStart -> Parser ann a -> Args -> m (a, Args)
...
runParserStep :: forall ann m a. MonadP ann m => ArgPolicy -> Parser ann a -> String -> Args -> m (Maybe (Parser ann a), Args)
runParserStep policy p arg args = do
  prefs <- getPrefs (Proxy @ann)
  flip runStateT args
    $ disamb (not (prefDisambiguate prefs))
    $ stepParser prefs policy arg p

parseError :: forall ann m x a. MonadP ann m => String -> Parser ann x -> m a
parseError arg = errorP (Proxy @ann) . UnexpectedError arg . SomeParser

runParserInfo :: MonadP ann m => ParserInfo ann a -> Args -> m a
runParserInfo i = runParserFully (infoPolicy i) (infoParser i)

runParserFully :: forall ann m a. MonadP ann m => ArgPolicy -> Parser ann a -> Args -> m a
runParserFully policy p args = do
  (r, args') <- runParser policy CmdStart p args
  case args' of
    []  -> return r
    a:_ -> parseError a (pure () :: Parser ann ())

-- | The default value of a 'Parser'.  This function returns an error if any of
-- the options don't have a default value.
evalParser :: Parser ann a -> Maybe a
evalParser (NilP r) = r
...
mapParser :: (forall x. ArgumentReachability -> Option ann x -> b)
          -> Parser ann a -> [b]
...
treeMapParser :: (forall x. ArgumentReachability -> Option ann x -> b)
          -> Parser ann a
          -> OptTree b
treeMapParser g = simplify . go False g
  where
    has_default :: Parser ann a -> Bool
    has_default p = isJust (evalParser p)

    go :: Bool
       -> (forall x. ArgumentReachability -> Option ann x -> b)
       -> Parser ann a
       -> OptTree b
